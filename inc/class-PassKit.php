	...
	class PassKit{
		...
		*/
		public function get_templates() {
			...
			if(!$this->pk_query('template/list/full')){
				// Print error
		...
		*/
		...
		// {{{ reset_passes_for_template()
		/**
		 * Reset passes for template
		 *
		 * Resets all the passes for the template
		 *
		 * @author Patrick Kosterman <patrick*passkit*c0m>
		 * @category Validation and Retrieval
		 * @access public
		 * @param String $template name of the template to reset the passes for
		 * @return pk_result
		*/
		public function reset_passes_for_template($template) {
			// template is required
			if ($template == '') {
				// Print error
				$this->error('Valid template name required.',debug_backtrace());
			...
			// Send reset query to PassKit

			if($this->pk_query('template/'.urlencode($template).'/reset'))
			{
				return $this->pk_result;
			}
			else
